...
bool isSameTree(struct TreeNode* p, struct TreeNode* q) {
	if (p == NULL && q == NULL) {//两树都为空，判定相同
		return true;
	}
	if (p == NULL || q == NULL) {//再上一个if的前提下，其中一个不空，判定为false
		return false;
	}
	if (p->val != q->val) {//值不同，判定为false
		return false;
	}
	return isSameTree(p->left, q->left) && isSameTree(p->right, q->right);//递归判定左子树和右子树
}
